...
let Configuration, OpenAIApi, openai, apiKey;
...
let commandNames = Object.keys(commandRegistry);
...
class DiscBot {
  ...
  xbotAuth() {
    return new Promise((resolve, reject) => {
      if (!config.xbotToken) {
        let nokey_error =
          'Error: no xbot api key, get one at https://x-bot.live and add to config.json under "xbotToken"';
  ...
  async handleCommand(sender, rawCmdMessage) {
    ...
    for (const i of Array(chrs.length).keys()) {
      if (
        chrs[i].charCodeAt() - chatOffset < 256 &&
  ...
  async handleJoinLeave(pName, wasJoin = true) {
    try {
      ...
      if (
        allTrue(plr, platform, bannedDevice, notWhitelisted, wasJoin) &&
        dontDoAutomod != true
      ) {
        console.log(
          orange(
